<template>
  <!-- <div class="popup"> -->
    <div class="popup-inner">
      <slot/>
      <h4 class="popup-title">Are you sure to delete {{ name }}?</h4>
      <div class="buttons">
        <b-button class="save primary-back mt-2" variant="danger" :disabled="disabled" @click="deleteItem(pages, scope, id)">
          <div>Delete</div>
        </b-button>
      </div>
    </div>
...
export default {
  ...
  props: ['TogglePopup', 'name', 'scope', 'id', 'pages'],
  methods: {
    async deleteItem (pages, scope, id) {
      await this.$resource.resource(2, `/${scope}/${id}`)
