<template>
  ...
  <div class="popup-inner">
    ...
    <div>
      ...
      <input type="text" class="mb-2" v-model="discount" placeholder="Discount">

      <label class="label mb-2">category</label>
      <b-form-select class="form-control form-control mb-2" v-model="category" :options="this.categories"></b-form-select>
    </div>
    <div class="buttons">
...
export default {
  ...
  methods: {
    async add () {
      ...
      await this.$resource.resource(1, `/items/${this.id}`, fd)
  ...
  async mounted () {
    await this.$resource.resource(0, `/items/${this.id}`)
    await this.$store.dispatch('getRes')
